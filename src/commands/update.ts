...
export async function fetchJsonWithRetry(url: string, timeoutMs = 8000, retries = 2): Promise<any> {
    let lastErr: unknown;
...
export async function getLatestRef(): Promise<{ ref: string; source: 'release' | 'tag' | 'default' }> {
    try {
